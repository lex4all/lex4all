...
We have developed lex4all as a desktop application for Microsoft Windows,\footnote{Windows 7 or 8 (64-bit).} since it relies on the Microsoft Speech Platform \cite{mspsdk} as explained below.
%for speech recognition (see Section~\ref{sec:engine}).
The application and its source code are freely available 
%for download 
via GitHub.\footnote{[Link removed to preserve submission anonymity]} 
...
A major challenge we faced in engineering a user-friendly application based on the Salaam pronunciation-mapping algorithm \cite{Qiao10} was its long running time. 
...
For application debugging and research purposes, this long training time is a serious disadvantage.
