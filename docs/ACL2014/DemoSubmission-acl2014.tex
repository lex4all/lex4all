...
While they greatly reduce the amount of data needed to build new recognizers, %however, 
...
%\cite{Qiao10,Chan12} 
to find the best pronunciation(s) for each term. This requires a pre-trained
recognition engine for a HRL 
...
We note here that 
%to improve the system's time-efficiency and thus usability, 
our implementation of Salaam deviates slightly from that of \newcite{Qiao10}, 
improving the time-efficiency and thus usability of the system 
%of the system
(see Sec.~\ref{sec:backend}).
%as explained in Section~\ref{sec:backend}. 
...
Word recognition accuracy for Yoruba 
%(25~words, 2~speakers, 5~samples per word per speaker) 
using old (slower) and new (faster) implementations, with $p$-values from 
%paired two-tailed 
$t$-tests for significance of difference in means. 
Bold indicates highest accuracy.\label{tab:accuracy}}
\end{table}
...
Once the target vocabulary and training audio have been specified, and the additional options have been set if desired, users click the ``Build Lexicon'' button and specify the desired name and target directory of the lexicon file to be saved, 
...
takes the default audio input device as its %data 
...
%We also thank
Dietrich Klakow, Florian Metze, and 
