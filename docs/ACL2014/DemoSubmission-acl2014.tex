...
Compared to pronunciations hand-written by a linguist, pronunciations generated automatically by this algorithm yield substantially higher recognition accuracy \cite[Sec.~5.2]{Qiao10}. Qiao et al. further improve accuracy by training on samples from multiple speakers, and by creating a lexicon with multiple pronunciations for each word (i.e. the $n$-best results of the training algorithm instead of the single best result).
...
\begin{table}[tb]
\begin{center}
\begin{tabularx}{\columnwidth}{X l r r X}
\hline
 & & Old & New & $p$\\
\hline
\noalign{\smallskip}
