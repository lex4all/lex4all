...
%%OLD
%Though tools for quickly training or adapting recognizers for new languages exist (e.g. CMUSphinx\footnote{http://www.cmusphinx.org} or the Rapid Language Adaptation Toolkit\footnote{http://i19pc5.ira.uka.de/rlat-dev}), these typically require hours of training audio to produce effective models, data which is by definition not available for a LRL. Furthermore, naive users may struggle to effectively utilize even such tools, which still demand some understanding of speech/language technologies.
%%
%%NEW
...
While they greatly reduce the amount of data needed to build new recognizers, however, these approaches still require non-trivial amounts of speech and text in the target language, which may be an obstacle for very low- or zero-resource languages. Furthermore, even high-level tools such as RLAT still demand some understanding of linguistics/language technology, and thus may not be accessible to all users.
%%
...
%uses a Hindi speech interface which recognizes 79 terms.
...
%TODO related work on pronunciation generation
...
Given this grammar and one or more audio recordings of the %target 
term, \newcite{Qiao10} use an iterative training algorithm to discover the best pronunciation(s) for that term, one phoneme at a time. 
...
%can help create speech-driven applications in LRLs, as has been demonstrated in 
...
%, so that non-expert users can quickly and easily create lexicons to develop speech interfaces in LRLs using existing HRL recognizers. 
...
We have developed \textit{lex4all} as a desktop application for Microsoft Windows,\footnote{Windows 7 or 8 (64-bit).} since it relies on the Microsoft Speech Platform (MSP) %\cite{mspsdk} 
...
%SDK library. \footnote{\url{http://msdn.microsoft.com/en-us/library/microsoft.speech.recognition}}
%\cite{mspsdk}.
...
Once pronunciations for all terms in the vocabulary have been generated, the application outputs a pronunciation lexicon for the given terms as an XML file conforming to the Pronunciation Lexicon Specification\footnote{http://www.w3.org/TR/pronunciation-lexicon/} (Fig.~\ref{fig:lexicon}). This lexicon can then be directly included in a speech recognition application built using the MSP API or a similar toolkit.
...
We choose the MSP for its robustness and ease of use, as well as to maintain comparability with the work of \newcite{Qiao10} and \newcite{Chan12}. 
%, who also used Microsoft's server-side US English recognizer. 
...
Pronunciations (sequences of source-language phonemes) for each term in the target vocabulary are generated from the audio sample(s) of that term using the iterative Salaam algorithm %\cite[Sec.~4.1]{Qiao10}, 
(Sec.~\ref{sec:background}), which employs the source-language recognizer and a special recognition grammar. In a first pass, the algorithm finds the best candidate(s) for the first phoneme of the sample(s), then the first two phonemes in the second pass, and so on until a stopping criterion is met. 
...
%if the best sequence from the $i^{th}$ pass has a lower score than the best sequence of the ${i - 1}^{th}$ pass 
%(with the ${i - 1}^{th}$ results returned as the best pronunciations) 
...
After the iterative training has completed, the $n$-best pronunciation sequences (with $n$ specified by the user -- see Sec.~\ref{sec:options}) for each term are written to the lexicon, each in a \texttt{phoneme} element corresponding to the \texttt{grapheme} element containing the term's orthographic form (e.g. Fig.~\ref{fig:lexicon}).
...
%described by \newcite{Qiao10} 
The algorithm depends on a ``super-wildcard'' grammar that allows the recognizer to match each sample of a given term to a ``phrase'' of 0-10 ``words'', each word comprising any possible sequence of 1, 2, or 3 source-language phonemes \cite{Qiao10}. Given the 40 phonemes of US English, this gives over 65,000 possibilities for each word, resulting in a huge training grammar and thus a long processing time. For a 25-term vocabulary with 5 training samples per term, the process takes approximately 1-2 hours
...
The algorithm can still discover pronunciation sequences of an arbitrary length, since, in each iteration, the phonemes discovered so far are prepended to the super-wildcard grammar, such that the phoneme sequence of the first ``word'' in the phrase grows longer with each pass 
\cite
%[p.~4]
{Qiao10}. 
...
%\end{tabular}
%\end{center}
%\caption{Word recognition accuracy using US English recognizer on Yoruba audio (25~words, 2~speakers, 5~samples per word per speaker), with results of paired two-tailed $t$-test for significance.\label{tab:accuracy}}
%\end{table*}
...
%(see Section~\ref{sec:recording} for more details on audio input).
...
takes the user's default audio input device as its %data 
...
Secondly, users may train the lexicon using our modified, %much 
...
Finally, 
%as mentioned in Section~\ref{sec:discrimtrain}, 
...
%and it would be interesting to investigate different pairings of source and target languages.
